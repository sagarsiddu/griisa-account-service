...
//        for (UserRequestDTO dto : requests) {
//            try {
//                processSingleUser(dto);
//            } catch (ConstraintViolationException e) {
//                log.warn("Validation failed for user: {}, violations: {}", dto.getEmail(), e.getConstraintViolations());
//            } catch (Exception e) {
//                log.error("Failed to process user: {}", dto.getEmail(), e);
//            }
//        }
//    }
//
//    private void processSingleUser(UserRequestDTO dto) throws Exception {
//        validate(dto);
//
//        User user = userRepo.save(buildUser(dto));
//        String kycResult = kycService.performKYC(user, dto.getAadhaarNumber(), dto.getPanNumber()).get();
...
//        if ("SUCCESS".equalsIgnoreCase(kycResult)) {
//            Account acc = accountRepo.save(buildAccount(user));
//            log.info("Account created: {}", acc.getAccountNumber());
//        } else {
//            log.warn("KYC failed for user: {}, skipping account creation", dto.getEmail());
//        }
//    }
//
//    private void validate(UserRequestDTO dto) {
...
//
//    private Account buildAccount(User user) {
//        // Avoid UUID; generate numeric ID optimized for index
//        String acctNum = "ACCT" + Year.now().getValue() + String.format("%08d", user.getUserId());
//        return Account.builder()
//                .user(user)
//                .accountNumber(acctNum)
